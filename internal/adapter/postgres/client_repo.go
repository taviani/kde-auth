...
func (r *ClientRepo) Upsert(ctx context.Context, client domain.OAuthClient) error {
	...
	_, err := r.pool.Exec(ctx, `
		INSERT INTO oauth_clients (
			client_id, client_secret_hash, name, redirect_uris, access_mode, token_endpoint_auth_method, android_package
		)
	...
	`, client.ClientID, client.ClientSecretHash.String(), client.Name, client.RedirectURIs, mode, authMethod, client.AndroidPackage)
	return err
}
...
func scanClient(row clientScanner) (domain.OAuthClient, error) {
	...
	err := row.Scan(&c.ID, &c.ClientID, &c.ClientSecretHash, &c.Name, &c.RedirectURIs, &mode, &authMethod, &c.AndroidPackage)
